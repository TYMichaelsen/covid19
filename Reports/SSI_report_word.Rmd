...
left_join(name_clusters_old, by = "substitutions")
  name_clusters$cluster_no[is.na(name_clusters$cluster_no)] <- (max(name_clusters$cluster_no, na.rm = T) +1):
```
...
name_clusters <- meta_ll %>%
  ...
  mutate(cluster_size  = paste0(sum(cluster_count)),
         cluster_start = min(as.Date(cluster_start), na.rm = T),
         cluster_end   = max(as.Date(cluster_end), na.rm = T))
...
```{r, fig.height = 8, fig.width=7, dpi=300}
...
```{r, fig.height=10, fig.width = 12, dpi=300}
