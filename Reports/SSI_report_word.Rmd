...
name_clusters <- meta_ll %>%
  ...
  summarise(cluster_aa_mutation = dplyr::first(direct_aa_mutations),
            cluster_count = n(), 
...
# Dump cluster names
write_csv(name_clusters, path = file.path(dirname(inputFile),paste0(format(Sys.Date(),"%d-%m-%Y"),"_cluster_definition.csv")))
#write_csv(name_clusters, path = file.path("/srv/rbd/rfb/R_scripts/Report",paste0(format(Sys.Date(),"%d-%m-%Y"),"_cluster_definition.csv")))
...
gg <- meta_ll %>%
  filter(!is.na(direct_mutations)) %>%
  group_by(direct_mutations) %>%
  summarise(n_aa = length(unique(direct_aa_mutations)),
            aa = dplyr::first(direct_aa_mutations)) %>% 
  arrange(-n_aa)
```
...
nDirMutOld <- meta_all %>% filter(date_linelist < date_rng[1] & 
                                    !is.na(direct_mutations) 
                                  & duplicated(direct_mutations) == F) %>% nrow()
```

I denne rapport indgår data fra perioden `r format(min(meta_all$date_linelist), '%d-%m-%Y')` til `r format(date_rng[2], '%d-%m-%Y')`. Der er særligt fokus på uge `r weeks` (`r format(date_rng[1], '%d-%m-%Y')` til `r format(date_rng[2], '%d-%m-%Y')`). Data der anvendes i denne rapport inkluderer:
...
clade_freq <- meta_all %>% 
  filter(date_linelist >= date_rng[1]  & date_linelist <= date_rng[2] & !is.na(genome_qc)) %>% 
...
d_clades_week <- filter(meta_all, !is.na(clade) & date_linelist <= date_rng[2]) %>%
