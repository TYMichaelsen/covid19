...
```{r read_old_cluster, eval = T}
# Read cluster definitions
cluster_files <- list.files("/srv/rbd/covid19/metadata/cluster_definition",pattern = "_definition", full.names = T)
name_clusters_old <- read_csv(cluster_files[length(cluster_files)]) %>% 
  select(substitutions, cluster_no)
```


```{r gen_new_cluster}
# Summarize clusters and merge old definitions
name_clusters <- meta_ll %>%
  filter(!is.na(substitutions)) %>%
  group_by(substitutions) %>%
  summarise(cluster_aa_mutation = dplyr::first(aa_mutations),
            cluster_size = n(), 
            cluster_start = min(as.Date(date_linelist), na.rm = T),
            cluster_end = max(as.Date(date_linelist), na.rm = T)) %>%
  arrange(cluster_start) %>% 
  left_join(name_clusters_old, by = "substitutions")

# Add numbers for new clusters
name_clusters$cluster_no[is.na(name_clusters$cluster_no)] <- 
  (1:length(name_clusters$cluster_no[is.na(name_clusters$cluster_no)]) + max(name_clusters$cluster_no, na.rm = T))
  
# Add cluster name
name_clusters <- name_clusters %>% 
  mutate(cluster_name = paste0("Cluster ",cluster_no)) 
# Dump cluster names
write_csv(name_clusters, path = file.path("/srv/rbd/covid19/metadata/cluster_definition/",
                                          paste0(format(Sys.Date(),"%Y-%m-%d"),"_cluster_definition.csv")))
```


```{r merge_cluster}
meta_ll <- left_join(meta_ll,name_clusters,by = "substitutions") %>% 
...
```{r plot_clusters_time, fig.height=10, fig.width = 12, dpi=300}
...
```{r tab_clusters}
# Per region.
meta_table <- left_join(clade_freq,meta_ll, by = "cluster_no") %>% 
  filter(date_linelist >= date_rng[1]  & date_linelist <= date_rng[2] & !is.na(Region) & !is.na(cluster_no)) %>% 
...
# Hele DK.
heleDK <- left_join(clade_freq,meta_ll, by = "cluster_no" )%>% 
  filter(date_linelist >= date_rng[1]  & date_linelist <= date_rng[2] & !is.na(cluster_no)) %>% 
  group_by(cluster_no) %>% 
  summarize(n = n(),
            pct_M = sum(Sex == "M") / n,
            pct_DK = sum(CitizenshipText == "Danmark") / n,
...
p <- ggplot(data = meta_map, aes(x= zip_lon, y = zip_lat, color = cluster_no)) +
