...
Description:
  ...
  Depends on finding the columns 'gisaid_id', 'library_id','curate_exclude','location', and 'sample_date' in the metadata. 
  These should be formatted as:
    ...
    location       = column with sample location.
...
#META=/srv/rbd/covid19/upload/2020-11-03-12-45_upload/tmp_metadata.tsv
#SEQS=/srv/rbd/covid19/genomes/2020-10-31-22-54_export/sequences.fasta
#WORKFLOW_PATH=/srv/rbd/tym/covid19/workflow
...
awk -F '\t' 'NR == 1 {for (i=1; i<=NF; i++) ix[$i] = i} NR > 1 {print $ix["ssi_id"]"\t"$ix["library_id"]"\t"$ix["gisaid_id"]"\t"$ix["location"]"\t"$ix["sample_date"]}' $META > tmp && mv tmp $META

# Only work with samples that are represented in both metadata and sequences.
grep ">" $SEQS | sed 's/>//' | sort > $OUTDIR/tmp_seqIDs
cut -f 1 $META | sort > $OUTDIR/tmp_metaIDs

comm -12 $OUTDIR/tmp_seqIDs $OUTDIR/tmp_metaIDs > $OUTDIR/tmp_intersect.txt
#comm -23 $OUTDIR/tmp_seqIDs $OUTDIR/tmp_metaIDs > $OUTDIR/only_in_seq.txt
#comm -13 $OUTDIR/tmp_seqIDs $OUTDIR/tmp_metaIDs > $OUTDIR/only_in_meta.txt

# Subset metadata.
awk -F'\t' 'FNR == NR {entries[$1]=$0; next} {if ($1 in entries) {print entries[$1]} else {print $1" had no matching entry, excluding." > "/dev/stderr"}}' $META $OUTDIR/tmp_intersect.txt > tmp && mv tmp $META
...
#head -n 10 $OUTDIR/tmp_toparallel.txt > tmp && mv tmp $OUTDIR/tmp_toparallel.txt
...
echo -e "gisaid_id\tlocation\tsample_date\tmd5sum\tfastq_file" > $OUTDIR/metadata.tsv
join <(awk -F'\t' '{ print $3"\t"$4"\t"$5 }' $OUTDIR/tmp_metadata.tsv | sort) <(sort $OUTDIR/tmp_md5sums.tsv) -t $'\t' >> $OUTDIR/metadata.tsv
