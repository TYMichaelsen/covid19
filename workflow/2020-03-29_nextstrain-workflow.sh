...
if [ -z ${META+x} ]; then echo "WARNING: -m missing, parts of the pipeline will not be performed."; fi;
if [ -z ${SEQS+x} ]; then echo "-s $MISSING"; exit 1; fi;
if [ -z ${OUTDIR+x} ]; then OUTDIR=$PWD; fi;
if [ -z ${THREADS+x} ]; then THREADS=50; fi;

### Code.----------------------------------------------------------------------
#SEQS=AAU-data/artic/aau-rename.fasta
#META=/srv/rbd/covid19/data/metadata_SSI/SSI_nextstrain.tsv
#OUTDIR=results_AAU
#THREADS=100
...
python3 GISAID-data/ncov/scripts/mask-alignment.py \
