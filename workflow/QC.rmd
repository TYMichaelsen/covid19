...
batch   <- "CJ024"
labmeta <- "CJ024/CJ024_metadata_sequencing.csv"
```
...
clades <- fromJSON(file = paste0(batch,"/QC/aligntree/clades.json")) %>%
  .$nodes %>%
  sapply("[[",1) %>%
...
write_delim(cladedump,path = paste0(batch,"/final_output/clades.tsv"),delim = "\t")
