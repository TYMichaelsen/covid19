...
# For build mode.
batch      <- "CJ061"
labmeta    <- "/srv/rbd/covid19/processing/CJ061/sample_sheet.csv"
input_dir  <- "/srv/rbd/covid19/processing/CJ061"
...
clades <- rjson::fromJSON(file = paste0(input_dir,"/QC/aligntree/clades.json")) %>%
  .$nodes %>%
  sapply("[[",1) %>%
...
d <- d %>%
  ...
  mutate(genome_qc = ifelse(coverage > 5 & genome_qc == "Fail","Dropout",genome_qc)) %>%
  mutate(genome_qc = factor(genome_qc,levels = c("HQ","MQ","Fail","Dropout","Fail_artic"))) 
```
...
- `Fail`: A genome with either >5000 N's in total.
- `Dropout`: A genome .
- `Fail_artic`: No genome generated with >5x coverage. The artic pipeline failed for some reason. 
...
rbind(tab,tot) %>%
  mutate(genome_qc = factor(genome_qc,levels = c("HQ","MQ","Fail","Dropout","Fail_artic","Total"))) %>%
  arrange(genome_qc) %>%
...
## amplicon dropout vs. library concentration

```{r}
ggplot(d,aes(x = dropout_1,y = dropout_2,colour = library_conc)) +
  geom_point()
```


## Library concentration vs. amplicon dropout

```{r}
cols <- c("#52854C", "#E7B800", "#FC4E07","#999999","black") %>%
  setNames(.,nm = c("HQ","MQ","Fail","Dropout","Fail_artic"))

ggplot(d,aes(x = library_conc,y = dropout,color = genome_qc)) + 
  geom_point() +
  labs(y      = "Amplicon dropout [n] ",
...
## Samples dropping out

```{r}
filter(d,genome_qc == "Dropout") %>%
  select(library_id,library_conc,coverage) %>%
  arrange(-coverage) %>%
  kable() %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = F)
```
